...
| 전투 이벤트 스트림 | 구현 | skill·critical·kill·bossVictory·defeat 고정 순서, 단일·분할 ID, 최근 100개, MAX_SAFE 초과 cursor, 저장 비영속 |
...
| 시각 회귀 | 구현 | Ubuntu 24.04·고정 Chromium/font/time/seed에서 전투 4 fixture × 4 viewport/motion variant, state hash, overflow·motion, screenshot diff artifact |
...
IRPG-106 기준선은 Vitest 17파일·113테스트, 자산 validator 21테스트, 일반 Playwright 12테스트와 production 자산 2테스트다. 전투 이벤트 전용 16테스트는 고정 ordinal·즉시 snapshot·동료 마무리·단일/분할 merge·100개 상한·MAX_SAFE 초과 cursor·collision 방어를 포함하며, persistence와 hook 테스트가 offline aggregate 유지 및 bootstrap·reset·import queue 폐기를 검증한다.
...
IRPG-408 기준선은 Vitest 23파일·153테스트, 자산 validator 25테스트, 일반 Playwright 23테스트, production 자산 Playwright 3테스트와 screenshot 20개다. 지역 3개의 경계·현재/완료/최전선/잠김·boss 파생, click/Enter/Space 차단, roving tab·Arrow/Home/End/Page 이동, 360px·200% 확대, corrupt region fallback, 자동 전투 중 focus 불변, disclosure 전 region 요청 0과 활성 지역 단독 lazy-load를 포함한다. `visual.map.stage-frontier`는 stage/highestStage 105에서 4개 반응형·모션 variant를 추가하며 Ubuntu canonical 체크인과 최종 CI 증거가 확인될 때 Done으로 전환한다.
...
| 결정론적 전투 이벤트 | 5종 event union, 고정 ordinal·ID, 발생 직후 snapshot, bounded merge, offline aggregate·비영속 저장 | IRPG-411 전투 로그와 IRPG-410 승패 결과 UI |
...
| 시각 자산 | manifest validator fixture, 자산 컴포넌트 load/error fallback, production URL·gzip·lazy-load Playwright, 적·보스 8종 stable mapping·360px·200%·A/B 저장 지속 | 최종 적·보스 일러스트 독립 미술 검토 P0·P1·P2 0 완료; 후속 지역·카드·결과 화면은 각 티켓에서 검토 |
...
로컬 전체 게이트는 `npm run verify`다. 브라우저 제외 게이트는 `npm run verify:code`다. canonical screenshot 비교는 Ubuntu GitHub Actions 전용이며 로컬에서는 건너뛰고, Ubuntu CI에서는 동일 전체 게이트가 16개 baseline을 반드시 비교한다. CI는 Chromium과 시스템 의존성을 설치한 뒤 worker 1개로 실행하며, 실패한 Playwright trace·screenshot·video와 HTML report를 artifact로 보존한다. 티켓을 Done으로 옮길 때 명령, 통과 결과, 필요한 수동 증거를 `Test evidence`에 남긴다.
