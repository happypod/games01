...
```ts
createInitialState(now): GameState
...
- 현재 엔진에는 무작위가 없어 동일 상태와 경과 시간은 항상 동일 결과를 만든다. 치명타·드롭을 추가할 때는 저장 가능한 시드 RNG가 선행되어야 한다.
...
- v2 envelope: `{ formatVersion: 2, revision, savedAt, state }`
...
schema migration registry는 IRPG-104의 저장형 RNG state 추가와 함께 확장한다. portable schema 1 fixture는 이후 버전에서도 회귀 대상으로 유지한다.
