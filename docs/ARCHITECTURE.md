...
- 유형 2의 원정 선택 오버레이 열림 여부는 저장하지 않는 로컬 disclosure 상태다. pending 존재만으로 전장을 가리거나 scene consumer를 멈추지 않으며, 사용자가 명시적으로 펼친 동안만 전장 base를 inert로 만들고 새 scene을 표시 없이 소비해 닫은 뒤 재생하지 않는다. mount·reload·layout 전환·새 pending 도착은 모두 접힘을 유지하고 가시적 건수 토글과 별도 polite status만 갱신한다.
