...
| 4A | IRPG-408 | Test | 3지역 맵과 roving keyboard 스테이지 탐색 |
...
`1A`와 `1B`, `2A`~`2C`, `4A`~`4C`는 선행 조건이 충족되면 병렬로 진행할 수 있다. 다만 한 변경 묶음에서는 Ready 티켓 하나씩 Review → Verify → Test를 통과시킨다. IRPG-506의 체크인된 Ubuntu canonical baseline 16개와 최종 push·PR 품질 게이트가 완료되었다. IRPG-408은 3지역 맵 구현·로컬 검증·독립 미술/접근성 review를 통과했고 Ubuntu canonical baseline 20개와 최종 CI를 확인하는 Test 단계다.
