...
| 4B | IRPG-409 | In Progress | 무기·갑옷·부적과 스킬의 일러스트 카드 |
...
`1A`와 `1B`, `2A`~`2C`, `4A`~`4C`는 선행 조건이 충족되면 병렬로 진행할 수 있다. 다만 한 변경 묶음에서는 Ready 티켓 하나씩 Review → Verify → Test를 통과시킨다. IRPG-408의 3지역 맵, 독립 미술/접근성 review, Ubuntu canonical baseline 20개, 최종 push·PR 품질 게이트가 완료되었다. IRPG-409는 카드 상태·고유 자산·지연 로드·시각 fixture 계약을 Ready로 고정했으며 구현 게이트를 진행한다.
