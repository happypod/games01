...
`1A`와 `1B`, `2A`~`2C`, `4A`~`4C`는 선행 조건이 충족되면 병렬로 진행할 수 있다. 다만 한 변경 묶음에서는 Ready 티켓 하나씩 Review → Verify → Test를 통과시킨다. 다음은 IRPG-506을 Ready로 정제해 기본 전투 screenshot harness와 CI artifact 계약을 구현한다.
