...
| 8 | IRPG-412 | Done | pending 원정 선택 이벤트 일러스트 카드 UI |
...
IRPG-206 결정론적 재도달 기준선은 통과했다. IRPG-205는 실제 참여자 모집·동의·관찰이 필요한 외부 검증이며 자동 세션으로 대체하거나 완료로 추정하지 않는다. IRPG-207은 `boss-milestone-v1` 골드 공식·30-bit 원장·무소급 schema4 migration과 전체 원격 게이트를 완료했다. IRPG-410은 비영속 결과 queue·좌표 dedupe·사용자 요청형 dialog·보상 비변경·고유 승패 art, Ubuntu canonical 40개와 3회 반복 120/120까지 완료했다. IRPG-107은 10단계 단위 최초 진입, 3종 shuffle-bag, 골드·HP 회복 전용 효과, pending 3개 상한, 환생 폐기, 무소급 schema5 migration과 전체 원격 게이트를 완료했다. IRPG-412는 카드별 lazy art·저장 read-back 뒤 exact-once 선택·focus 복원·fallback, Ubuntu canonical 48/48·3회 반복 144/144, 체크인 기준선의 push·PR quality gate까지 완료했다.
...
- 아이템은 고정 `ITEM_REGISTRY` 기반으로 ID 수량만 저장소에 저장하며, 3공간 인벤토리(전리품 가방, 캠프 보관함, 캐릭터 가방) 및 부위별 장착, 조합형 스킬 슬롯을 수용한다.
