...
`1A`와 `1B`, `2A`~`2C`, `4A`~`4C`는 선행 조건이 충족되면 병렬로 진행할 수 있다. 다만 한 변경 묶음에서는 Ready 티켓 하나씩 Review → Verify → Test를 통과시킨다. IRPG-408의 3지역 맵, 독립 미술/접근성 review, Ubuntu canonical baseline 20개, 최종 push·PR 품질 게이트가 완료되었다. 다음은 IRPG-409의 카드 상태·자산·시각 fixture 계약을 Ready로 정제해 구현한다.
